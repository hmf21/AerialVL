    ...
    for query_index, pred in enumerate(predictions):
        for i, n in enumerate(args.recall_values):
            if np.any(np.in1d(pred[:n], positives_per_query[query_index])):
                ...
                if n == 1 and args.save_retrieval_result:
                    paired_prediction_path = eval_ds.database_paths[pred[0]]
                    paired_query_path = eval_ds.queries_paths[query_index]
                    paired_groundtruth_path = eval_ds.database_paths[positives_per_query[query_index][0]]
                    os.mkdir('./result/'+str(query_index))
                    _ = shutil.copy(paired_prediction_path, './result/{}/pred_pic.png'.format(query_index))
                    _ = shutil.copy(paired_groundtruth_path, './result/{}/gt_pic.png'.format(query_index))
                    _ = shutil.copy(paired_query_path, './result/{}/query_pic.png'.format(query_index))
                break
...
def test_two_stage(args, eval_ds, model, test_method="hard_resize", pca=None):
    """Compute features of the given dataset and compute the recalls."""
    ...
    reranked_predictions = rerank(predictions, eval_ds, args)
    ...
    # print the
    rerank_recalls = np.zeros(len(args.recall_values))
    for query_index, pred in enumerate(reranked_predictions):
        for i, n in enumerate(args.recall_values):
            if np.any(np.in1d(pred[:n], positives_per_query[query_index])):
                ...
                break
    # Divide by the number of queries*100, so the recalls are in percentages
    rerank_recalls = rerank_recalls / eval_ds.queries_num * 100
    rerank_recalls_str = ", ".join([f"R@{val}: {rec:.1f}" for val, rec in zip(args.recall_values, rerank_recalls)])

    return recalls, recalls_str, rerank_recalls, rerank_recalls_str
