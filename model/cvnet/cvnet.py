...
from model.cvnet.cvnet_utils import extract_feat_res_pycls, get_configs
from model.cvnet.cvnet_block import CVLearner, Correlation
from model.cvnet.resnet import ResNet
from model.cvnet.utils.create_model import create_model as create_model_
...
def create_model(
    ...
    model = create_model_(CVNet, configs=configs, pretrained=pretrained, checkpoint_path=checkpoint_path, device=device)
    return model


# if __name__ == '__main__':
#     path = '.checkpoint/CVPR2022_CVNet_R50.pyth'
...
#     key_image = torch.randn(1, 3, 224, 224).to(device)
#
