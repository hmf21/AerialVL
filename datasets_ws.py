...
def path_to_pil_img(path):
    ...
    # return Image.fromarray(cv2.imread(path)).convert('RGB')
...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="VPR", split="train"):
        super().__init__()
        self.args = args
        ...
        # Change the name of folder to match the dataset format
        database_folder = join(self.dataset_folder, "map_database")
        ...
        self.database_paths = sorted(glob(join(database_folder, "**", args.img_ext), recursive = True))
        # self.queries_paths = sorted(glob(join(queries_folder, "**", args.img_ext), recursive = True)[::10])
        self.queries_paths = sorted(glob(join(queries_folder, "**", args.img_ext), recursive = True))
        # The format must be path/to/file/@utm_easting@utm_northing@...@.jpg
        # We change the code to fit the image format with path/to/file/@utm_easting@utm_northing.***
        database_utms_LT = np.array([(path.split("@")[2], path.split("@")[3]) for path in self.database_paths]).astype(float)
        database_utms_RB = np.array([(path.split("@")[4], path.split("@")[5]) for path in self.database_paths]).astype(float)
        self.database_utms = (database_utms_LT + database_utms_RB) / 2
        self.queries_utms = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.queries_paths]).astype(float)

        # Find soft_positives_per_query, which are within val_positive_dist_threshold (deafult 25 meters)
        # we use longitude and latitude to present the image
        # therefore we set the val_positive_dist_threshold as
    ...
    def __getitem__(self, index):
        ...
        # to cope with the different situation of the database images and the query images
        # we treat differently to these two kind of images
        ...
        if index < self.database_num:
            if self.test_method == "hard_resize":
                ...
                # for AerialVL, the map patches in the database should
...
class TestDataset(data.Dataset):
    """Dataset with images, used for extract features
    """

    def __init__(self, args, queries_folder, database_folder):
        super().__init__()
        self.args = args
        self.queries_folder = queries_folder
        self.database_folder = database_folder
        if self.queries_folder is not None:
            self.queries_folder = join(queries_folder)
            self.database_folder = join(database_folder)
            ...
            database_utms_LT = np.array([(path.split("@")[2], path.split("@")[3]) for path in self.database_paths]).astype(
                float)
            database_utms_RB = np.array([(path.split("@")[4], path.split("@")[5]) for path in self.database_paths]).astype(
                float)
            self.database_utms = (database_utms_LT + database_utms_RB) / 2
            self.queries_utms = np.array([(path.split("@")[2], path.split("@")[3]) for path in self.queries_paths]).astype(float)
